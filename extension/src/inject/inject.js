...
const noTokenText =
  browser.i18n.getMessage("noTokenBtn") || "Set a valid token in config";
...
function generateCsvFile(data) {
  if (data) {
    let csvContent =
      "Organization,Repository,Repository URL,Result file URL,Results\n";

    data.forEach((item) => {
      const matched = item.text_matches.map((match) =>
        match.fragment.replace(/(?:\r\n|\r|\n|,|\s{2,})/g, " ").toString()
      );
      csvContent += `${item.repository.owner.login},${
        item.repository.name
      },${encodeURI(item.repository.html_url)},${encodeURI(
        item.html_url
      )},${matched}\n`;
    });
 ...
 * @returns
 */
async function search(page = 1, resultsCount = 0, items = []) {
  ...
  try {
    ...
    if (resultsCount < data.total_count) {
      search(page + 1, resultsCount, items);
    } else {
      // No more pages, export file
...
function onDownload(event) {
  getToken((token) => {
    downloadButton.querySelector("span").textContent =
      browser.i18n.getMessage("downloadingStatus") || "Downloading...";
    downloadButton.disabled = true;
    textMatchHeaders.append("Authorization", "token " + token);
